...
interface RefineInput {
  ...
  panel: {
    ...
    duration: number | null  // giây - quyết định motion intensity
    srtSegment: string | null  // có narrator → duration ngắn
    characters: Array<{
      name: string
...
- ⚠️ Motion intensity phải khớp với panel.duration:
  - duration ≤ 2s hoặc có srtSegment (narrator đọc): chỉ micro-motion
    (chớp mắt, thở nhẹ, tay run, môi mấp máy) — KHÔNG quay đầu, bước đi
