...
import LocalServicesTab from './components/LocalServicesTab'
...
export default function ProfilePage() {
  ...
  // 主要分区：扣费记录 / API配置 / 本地服务
  const [activeSection, setActiveSection] = useState<'billing' | 'apiConfig' | 'localServices'>('apiConfig')
          ...
          <div className="w-64 flex-shrink-0">
            <div className="glass-surface-elevated h-full flex flex-col p-5">
              ...
              <nav className="flex-1 space-y-2">
                ...
                <button
                  onClick={() => setActiveSection('localServices')}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all cursor-pointer ${activeSection === 'localServices'
                    ? 'glass-btn-base glass-btn-tone-info'
                    : 'text-[var(--glass-text-secondary)] hover:bg-[var(--glass-bg-muted)]'
                    }`}
                >
                  <AppIcon name="server" className="w-5 h-5" />
          ...
          <div className="flex-1 min-w-0">
            <div className="glass-surface-elevated h-full flex flex-col">
              ...
              {activeSection === 'apiConfig' ? (
                <ApiConfigTab />
              ) : activeSection === 'localServices' ? (
                <div className="h-full overflow-y-auto p-6"><LocalServicesTab /></div>
              ) : (
