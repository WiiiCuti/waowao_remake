...
export function usePanelVoiceManager({
  ...
  // Lip sync only applies to character dialogue, not narrator voice-over
  const characterVoiceLines = localVoiceLines.filter((line) => !line.isNarration && line.speaker !== 'Narrator')
  const hasCharacterVoiceLines = characterVoiceLines.length > 0
  const hasCharacterAudio = characterVoiceLines.some((line) => line.audioUrl)
