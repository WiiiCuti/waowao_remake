                        ...
                        {voiceManager.localVoiceLines.map((voiceLine) => {
                          ...
                          return (
                            <div key={voiceLine.id} className="flex items-start gap-1.5 p-1.5 bg-[var(--glass-bg-muted)] rounded text-[10px]">
                              ...
                              <div className="flex-1 min-w-0">
                                {((voiceLine as unknown) as { isNarration?: boolean }).isNarration || voiceLine.speaker === 'Narrator' ? (
                                  <>
                                    <span className="text-purple-400 inline-flex items-center gap-1 mr-1">
                                      <AppIcon name="bookOpen" className="w-3 h-3" />
                                      {t('voice.narrator', { fallback: '旁白 (Narrator)' })}:
                                    </span>
                                    <span className="text-[var(--glass-text-secondary)]">{voiceLine.content}</span>
                                  </>
                                ) : (
                                  <>
