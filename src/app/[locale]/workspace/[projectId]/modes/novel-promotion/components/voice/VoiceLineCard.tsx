...
export default function VoiceLineCard({
    ...
    const isNarration = line.isNarration || line.speaker === 'Narrator'

    return (
        <div
            className={`relative glass-surface-elevated overflow-hidden transition-all hover:shadow-[var(--glass-shadow-md)] hover:-translate-y-0.5 ${line.audioUrl ? 'ring-1 ring-[var(--glass-focus-ring)]/60' : hasVoice ? (isNarration ? 'ring-1 ring-purple-400/50' : '') : 'ring-1 ring-[var(--glass-stroke-warning)]/60'
                }`}
        >
            {/* 顶部：播放/生成区域 */}
            ...
            {/* 序号标签 */}
            <div className={`absolute top-2 left-2 backdrop-blur-sm text-white px-2 py-0.5 rounded-lg text-xs font-medium ${isNarration ? 'bg-purple-500/80' : 'bg-[var(--glass-overlay)]'}`}>
                {isNarration ? `#Narrator #${line.lineIndex}` : `#${line.lineIndex}`}
            </div>
            ...
            <div className="px-4 py-2.5 bg-[var(--glass-bg-muted)]/50 border-t border-[var(--glass-stroke-base)]/60 flex items-center justify-between gap-2">
                <span className={`inline-flex items-center gap-1 px-2.5 py-1 ${isNarration ? 'bg-purple-500/20 text-purple-400' : 'bg-[var(--glass-tone-info-bg)]/80 text-[var(--glass-tone-info-fg)]'} text-xs rounded-lg truncate max-w-[160px] font-medium`} title={line.speaker}>
                    {isNarration ? <><AppIcon name="bookOpen" className="w-3.5 h-3.5" />旁白 (Narrator)</> : line.speaker}
                </span>
                {hasVoice ? (
