            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  disabled={editingSpeaker === 'Narrator'}
                  className="w-full rounded-xl border border-[var(--glass-stroke-strong)] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--glass-tone-info-fg)] resize-y disabled:opacity-50"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(event) => {
                    const spk = event.target.value
                    onEditingSpeakerChange(spk)
                    if (spk === 'Narrator' && editingMatchedPanelId) {
                      const panel = bindablePanelOptions.find(p => p.id === editingMatchedPanelId)
                      if (panel?.srtSegment) onEditingContentChange(panel.srtSegment)
                    }
                  }}
                  disabled={!!editingLineId && editingSpeaker === 'Narrator'}
                  className="w-full rounded-xl border border-[var(--glass-stroke-strong)] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--glass-tone-info-fg)] disabled:opacity-50"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    onEditingMatchedPanelIdChange(id)
                    if (editingSpeaker === 'Narrator') {
                      const panel = bindablePanelOptions.find(p => p.id === id)
                      if (panel?.srtSegment) onEditingContentChange(panel.srtSegment)
                    }
                  }}
