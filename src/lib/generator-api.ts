...
import { ComfyUIImageGenerator } from './generators/image/comfyui'
import { ComfyUIVideoGenerator } from './generators/video/comfyui'
...
export async function generateImage(
    ...
    // ComfyUI uses its own /prompt API, bypass openai-compat gateway entirely.
    // Detected by port 8188 or 'comfyui' in the base URL.
    const isComfyUIEndpoint = (providerConfig.baseUrl?.includes(':8188') || 
                              providerConfig.baseUrl?.includes('comfyui') ||
                              providerConfig.baseUrl?.includes('localhost') && providerConfig.baseUrl?.includes('8188'))
    _ulogInfo(`[generateImage] ComfyUI detection: providerKey=${providerKey}, baseUrl=${providerConfig.baseUrl || '(empty)'}, gatewayRoute=${providerConfig.gatewayRoute || 'default'}, isComfyUIEndpoint=${isComfyUIEndpoint}`)
    if (providerKey === 'comfyui' || isComfyUIEndpoint) {
        const { referenceImages, ...generatorOptions } = options || {}
        const generator = new ComfyUIImageGenerator(selection.provider)
...
export async function generateVideo(
    ...
    const isComfyUIEndpoint = (providerConfig.baseUrl?.includes(':8188') || 
                              ...
                              providerConfig.baseUrl?.includes('localhost') && providerConfig.baseUrl?.includes('8188'))
    if (providerKey === 'comfyui' || isComfyUIEndpoint) {
        const generator = new ComfyUIVideoGenerator(selection.provider)
        return await generator.generate({
            userId,
            imageUrl,
            prompt: options?.prompt,
