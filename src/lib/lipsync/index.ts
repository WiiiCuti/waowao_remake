...
import { submitComfyUILipSync } from '@/lib/lipsync/providers/comfyui-lipsync'
...
function resolveProviderKey(value: string): LipSyncProviderKey {
  ...
  if (providerKey === 'fal' || providerKey === 'vidu' || providerKey === 'bailian' || providerKey === 'comfyui') {
    return providerKey
  }
...
export async function generateLipSync(
  ...
  try {
    ...
    if (providerKey === 'comfyui') {
      const result = await submitComfyUILipSync(preprocessedParams, context)
      _ulogInfo(`[LipSync] ComfyUI (fake) 任务完成: ${result.requestId}`)
      return result
    }
