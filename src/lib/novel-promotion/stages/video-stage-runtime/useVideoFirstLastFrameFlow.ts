...
export function useVideoFirstLastFrameFlow({
  ...
  const firstLastFrameModelOptions = useMemo(() => {
    const withCapability = videoModelOptions.filter((option) => supportsFirstLastFrame(option))
    if (withCapability.length > 0) return withCapability
    // Fallback: if no model has the capability flag, use all video models
    // (e.g., custom providers like ComfyUI that support FL via workflow but lack capability metadata)
