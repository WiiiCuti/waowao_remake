...
export type SpeakerVoiceEntry = FalSpeakerVoiceEntry | BailianSpeakerVoiceEntry | OmniVoiceSpeakerVoiceEntry
...
export type VoiceGenerationBinding = FalVoiceGenerationBinding | BailianVoiceGenerationBinding | OmniVoiceVoiceGenerationBinding
...
function normalizeRawSpeakerVoiceEntry(raw: unknown, speaker: string): SpeakerVoiceEntry {
  ...
  if (provider === 'omnivoice') {
    if (!audioUrl) {
      throw new Error(`SPEAKER_VOICE_ENTRY_INVALID_OMNIVOICE_AUDIO: ${speaker}`)
    }
    return {
      provider: 'omnivoice',
      voiceType,
...
function normalizeProviderKey(providerKey: string): SupportedAudioProviderKey | null {
  ...
  if (providerKey === 'openai-compatible') return 'omnivoice'
  return null
}
...
function toOmniVoiceBinding(source: VoiceSource, referenceAudioUrl: string | null): OmniVoiceVoiceGenerationBinding | null {
  if (!referenceAudioUrl) return null
  return {
    provider: 'omnivoice',
    source,
...
export function resolveVoiceBindingForProvider(params: {
  ...
  if (providerKey === 'omnivoice') {
    const fromCharacter = toOmniVoiceBinding('character', characterAudioUrl)
    if (fromCharacter) return fromCharacter
    // Accept both omnivoice and fal (uploaded audio) bindings as valid reference for OmniVoice TTS
    if (params.speakerVoice?.provider === 'omnivoice') {
      return toOmniVoiceBinding('speaker', readTrimmedString(params.speakerVoice.audioUrl))
    }
...
export function hasAnyVoiceBinding(params: {
  ...
  if (params.speakerVoice.provider === 'fal' || params.speakerVoice.provider === 'omnivoice') {
...
export function getSpeakerVoicePreviewUrl(speakerVoice?: SpeakerVoiceEntry | null): string | null {
  ...
  if (speakerVoice.provider === 'fal' || speakerVoice.provider === 'omnivoice') {
