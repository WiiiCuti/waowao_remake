...
export function parseExternalId(externalId: string): {
    ...
    if (externalId.startsWith('COMFYUI:')) {
        const parts = externalId.split(':')
        const requestId = parts.slice(2).join(':')
        const encodedUrl = parts[1] === 'RESULT'
...
export async function pollAsyncTask(
    ...
    switch (parsed.provider) {
        ...
        case 'COMFYUI':
            return await pollComfyUITask(parsed.requestId)
        case 'SILICONFLOW':
...
async function pollComfyUITask(requestId: string): Promise<PollResult> {
    return {
        status: 'failed',
        error: `COMFYUI_POLL_NOT_IMPLEMENTED: COMFYUI fake lipsync does not support async polling (${requestId})`,
    }
}
